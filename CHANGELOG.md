...
- **EVPN-BGP 控制面（P1-1）**：`display evpn` / `display bgp evpn` 从纯只读占位升级为**真实配置态渲染**，补齐控制面配置模型与命令面（数据面 Type-2/3/5 路由仍诚实占位）：
  - **EVPN 实例 / Bridge Domain**：系统视图 `evpn vpn-instance <id>` 进入 `[<dev>-evpn-instance-<id>]`；视图内 `route-distinguisher <rd>`、`vpn-target <rt> [import|export|both]`（方向前缀编码，缺省 both）、`bridge-domain <bd-id>` 进入 `[<dev>-bd-<bd-id>]`；BD 视图内 `vxlan vni <vni>` 写 BD↔VNI；接口视图（如 `interface Vlanif10`）`bridge-domain <bd-id>` 绑定三层网关（写 `evpn:bd:<id>:vlanif`）。
  - **BGP L2VPN EVPN 地址族**：BGP 视图 `l2vpn-family evpn` 进入 `[<dev>-bgp-<as>-l2vpn-evpn]`；视图内 `peer <ip> enable`、`advertise irb`。
  - **quit 链三档显式列出**：`ViewBD → ViewEVPNInstance → ViewSystem`、`ViewL2VPNEvpn → ViewBGP`（嵌套子视图 quit 不得越级回 ViewSystem；L2VPN EVPN quit 回 BGP 时保留 `CurrentSub` 维持提示符）；`GetPrompt` 与前端 `CliTerminal.tsx buildPrompt` 同步三视图提示符。
  - **诚实占位**：运行态（Type-2/3/5 路由、邻居会话）恒 `-` + `evpnCtrlSimNote` 注记；`display evpn [vni|peer|routing-table|instance <id>]` / `display bgp evpn` 渲染真实配置态（实例/RD/RT/BD/VNI/peer），不编造邻居 IP / 路由条目。
  - **持久化**：DeviceConfig 精确前缀键 `evpn:instance:<id>:rd|rt|bd`、`evpn:bd:<id>:vni|vlan|vlanif`、`bgp:l2vpn-evpn:enabled|peer:<ip>:enabled|advertise-irb`；`save`/`reload` 经 `loadEVPNFromDeviceConfig` 重建；`display current-configuration` 汇出 EVPN 控制面配置块。
  - **兼容零回归**：系统视图 `route-distinguisher`/`vpn-target` 维持旧 `vxlan:*` 键（display vxlan 消费），上下文感知分派。
  - 接线遵循三件套：`evpn_ctrl_eval.go`（纯函数/键/校验/重建）+ `evpn_ctrl_cmd.go`（视图入口与子命令，仅写 DeviceConfig）+ `evpn_ctrl_display.go`（真实配置态渲染）；能力矩阵 `l3SwitchOnly()`（L2 设备守卫）；补全三视图关键字表并纳入 `TestCompletionNoDrift`；新增 `TestEVPNControlPlane*` 9 用例锁死（生命周期/display/quit 链/undo/L2 守卫/reload/旧键零回归/补全）。
  - **浏览器 e2e 实测**：`l3_switch` 拓扑 → `evpn vpn-instance 1`→`route-distinguisher 100:1`→`vpn-target 100:1 both`→`bridge-domain 10`→`vxlan vni 5010`→`bgp 65001`→`l2vpn-family evpn`→`peer 10.0.0.2 enable`→`advertise irb`→`display bgp evpn` 显示配置态 + 诚实占位注记，截图 `deliverables/p1-1-evpn-bgp-e2e.png`。
