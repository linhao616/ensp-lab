...
func main() {
	...
	srv := &http.Server{
		...
		// 超时加固（2026-08-12 发布前审计 P0-R3）：
		// 零值意味着「永不超时」，慢速发送请求头的连接会一直占用 goroutine 与
		// 文件描述符（Slowloris）。这里只设两个绝对安全的超时：
		//   ReadHeaderTimeout —— 只约束请求头读取，不影响正常请求体上传；
		//   IdleTimeout       —— 只约束 keep-alive 空闲连接的复用等待。
		//
		// 刻意不设 WriteTimeout：/api/sim/events 是 SSE 长连接，写超时会周期性
		// 掐断事件推送。也不设 ReadTimeout：请求体已由 maxBytesReaderMiddleware
		// 限制在 10MB，再叠加读超时对慢速客户端收益有限、却可能误伤大拓扑导入。
